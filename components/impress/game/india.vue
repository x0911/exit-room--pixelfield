<template>
  <v-container>
    <!-- :value="videos.intro.ended && model === 0" -->
    <v-dialog
      :value="videos.intro.ended && model === 0"
      overlay-opacity="0"
      persistent
      class="elevation-0 z-index-max max-height-100"
      content-class="elevation-0 z-index-max max-height-100"
      :retain-focus="false"
      scrollable
      fullscreen
    >
      <v-card class="transparent" tile flat>
        <v-card-text class="px-0 pb-2 info-screen v-card darken border-3">
          <v-system-bar color="primary-darken">
            <div
              :class="{ 'error-spotted': errorSpots.url }"
              @click="addErrorSpot('url')"
            >
              <v-tooltip bottom color="black">
                <template #activator="{ on }">
                  <span
                    v-on="on"
                    class="text-caption d-flex justify-center align-center"
                  >
                    <v-icon color="error">mdi-lock-open-variant-outline</v-icon>
                    http://topmedicine.com/
                  </span>
                </template>
                <span> http://topmedicine.com/ </span>
              </v-tooltip>
            </div>
            <v-spacer></v-spacer>
            <v-btn small icon @click="showHintDialog()">
              <v-icon small>mdi-help-circle-outline</v-icon>
            </v-btn>
            <v-btn small icon @click="showMapDialog()">
              <v-icon small>mdi-window-minimize</v-icon>
            </v-btn>
            <v-btn small icon @click="showMapDialog()">
              <v-icon small>mdi-window-close</v-icon>
            </v-btn>
          </v-system-bar>
          <v-app-bar color="primary-darken lighten-1">
            <v-avatar size="40">
              <v-img
                :src="require('@/assets/images/games/india/novertis-logo.png')"
              ></v-img>
            </v-avatar>
            <v-app-bar-title class="ms-4">TopMedicine</v-app-bar-title>
            <v-spacer></v-spacer>
            <v-toolbar-items>
              <v-btn color="transparent">
                {{ $t('india.patients') }}
              </v-btn>
              <v-btn color="transparent">
                {{ $t('india.healthcare') }}
              </v-btn>
              <v-btn color="transparent">
                {{ $t('india.news') }}
              </v-btn>
              <v-btn color="transparent">
                {{ $t('india.careers') }}
              </v-btn>
            </v-toolbar-items>
          </v-app-bar>
          <v-card
            class="transparent"
            tile
            flat
            height="calc(100vh / 5 * 2.5)"
            max-height="542"
          >
            <v-img
              position="bottom"
              :src="require('@/assets/images/games/india/banner.jpg')"
              height="100%"
              width="100%"
            >
              <!-- gradient="to top right, #b564c9d9, #09369da6" -->
              <v-layout fill-height align-end>
                <v-container class="pb-0">
                  <v-layout justify-center column align-center>
                    <v-card
                      width="100%"
                      max-width="1170"
                      class="d-flex transparent"
                    >
                      <v-sheet color="error" tile>
                        <v-card-title>
                          {{ $t('india.about-the-company') }}
                        </v-card-title>
                        <v-card-subtitle
                          :class="{
                            'error-spotted': errorSpots.poorGrammer,
                          }"
                          @click="addErrorSpot('poorGrammer')"
                        >
                          {{ $t('india.about-the-company-text') }}
                        </v-card-subtitle>
                      </v-sheet>
                    </v-card>
                  </v-layout>
                </v-container>
              </v-layout>
            </v-img>
          </v-card>
          <v-container>
            <v-layout justify-center column align-center>
              <v-card
                class="transparent"
                tile
                flat
                max-width="1170"
                width="100%"
              >
                <v-card-title>
                  <span class="f-title text-h6">
                    <span class="ls-5"> {{ $t('india.our-purpose-is') }} </span>
                  </span>
                </v-card-title>
                <v-card-text>
                  <v-layout>
                    <v-flex>
                      <v-list class="transparent" max-width="500">
                        <v-list-item class="wrap ps-0">
                          <v-list-item-content>
                            <v-list-item-subtitle class="lh-1-4">
                              {{ $t('india.some-text-one') }}
                            </v-list-item-subtitle>
                          </v-list-item-content>
                        </v-list-item>
                        <v-list-item class="wrap ps-0">
                          <v-list-item-content>
                            <v-list-item-subtitle class="lh-1-4">
                              {{ $t('india.some-text-two') }}
                            </v-list-item-subtitle>
                          </v-list-item-content>
                        </v-list-item>
                        <v-list-item class="wrap ps-0">
                          <v-list-item-content>
                            <v-list-item-subtitle
                              class="lh-1-4"
                              :class="{
                                'error-spotted': errorSpots.suspiciousContent,
                              }"
                              @click="addErrorSpot('suspiciousContent')"
                            >
                              {{ $t('india.some-text-three') }}
                              <v-tooltip bottom color="black">
                                <template #activator="{ on }">
                                  <span
                                    v-on="on"
                                    v-ripple
                                    class="orange--text font-weight-bold cursor-pointer"
                                  >
                                    {{ $t('india.link-here') }}
                                  </span>
                                </template>
                                <span> http://www.stealmoney.com </span>
                              </v-tooltip>
                            </v-list-item-subtitle>
                          </v-list-item-content>
                        </v-list-item>
                      </v-list>
                    </v-flex>
                    <v-flex>
                      <splatted-img
                        clip="splattier"
                        :src="
                          require('@/assets/images/games/india/company-history-5.jpg')
                        "
                      >
                      </splatted-img>
                    </v-flex>
                  </v-layout>
                </v-card-text>
                <div>
                  <v-card
                    :disabled="errorSpots.banner1"
                    class="mt-8 px-2 py-4"
                    color="rgba(255,255,255,0.1)"
                    max-width="510"
                  >
                    <v-card-title>
                      <span class="f-title text-h6">
                        <span class="ls-5 g__color-blinker">
                          {{ $t('india.update-required') }}
                        </span>
                      </span>
                    </v-card-title>
                    <v-card-text>
                      {{ $t('india.update-required-text-1') }}
                      <v-tooltip top color="black">
                        <template #activator="{ on }">
                          <span v-on="on" class="orange--text">
                            http://topmedicinex.com/update/
                          </span>
                        </template>
                        <span>
                          http://www.stealyourcredentials.com
                        </span> </v-tooltip
                      >, {{ $t('india.update-required-text-2') }}
                    </v-card-text>
                    <v-card-actions class="px-4">
                      <v-spacer></v-spacer>
                      <v-btn
                        text
                        class="px-4"
                        large
                        depressed
                        @click="showVerify(false)"
                      >
                        {{ $t('india.i-will-not-do-that') }}
                      </v-btn>
                      <v-btn
                        color="error"
                        class="px-6"
                        large
                        depressed
                        @click="showVerify(true)"
                      >
                        {{ $t('india.verify') }}
                      </v-btn>
                    </v-card-actions>
                  </v-card>
                </div>
                <div>
                  <v-row>
                    <v-col cols="12" lg="6">
                      <v-card
                        :disabled="errorSpots.banner2"
                        tile
                        flat
                        class="mt-8 text-center py-4"
                        color="rgba(255,255,255,0.1)"
                      >
                        <v-card-title class="d-block">
                          <span class="f-title text-h6">
                            <span class="ls-5">
                              {{ $t('india.big-offer') }}
                            </span>
                          </span>
                        </v-card-title>
                        <v-card-text>
                          <div>
                            {{ $t('india.big-offer-text-1') }}
                            <br />
                            {{ $t('india.big-offer-text-2') }}
                          </div>
                        </v-card-text>
                        <v-card-actions class="justify-center px-4">
                          <v-btn
                            text
                            class="px-4"
                            large
                            depressed
                            @click="getPromotion(false)"
                          >
                            {{ $t('final-task-trap.no-thanks') }}
                          </v-btn>
                          <v-btn
                            color="error"
                            class="px-6"
                            large
                            depressed
                            @click="getPromotion(true)"
                          >
                            {{ $t('india.get-promotion') }}
                          </v-btn>
                        </v-card-actions>
                      </v-card>
                    </v-col>
                    <v-col cols="12" lg="6">
                      <v-card
                        :disabled="errorSpots.insta"
                        tile
                        flat
                        class="mt-8 text-center py-4"
                        color="rgba(255,255,255,0.1)"
                      >
                        <v-card-title
                          class="d-flex justify-center align-center gap-2"
                        >
                          <v-icon size="28">mdi-instagram</v-icon>
                          <svg
                            class="insta__logo"
                            width="512px"
                            height="146px"
                            viewBox="0 0 512 146"
                            version="1.1"
                            xmlns="http://www.w3.org/2000/svg"
                            xmlns:xlink="http://www.w3.org/1999/xlink"
                            preserveAspectRatio="xMidYMid"
                          >
                            <g fill="#ffffff">
                              <path
                                d="M27.1265503,0.902013423 C16.6803758,5.27806711 5.19645638,17.6253423 1.56950336,33.1348188 C-3.02389262,52.7806711 16.0910604,61.0895034 17.6588456,58.3688591 C19.5041074,55.168 14.2337718,54.0855839 13.1487785,43.8919732 C11.7467919,30.7260134 17.8675973,16.014604 25.571651,9.55962416 C27.0028456,8.36037584 26.9349799,10.0295302 26.9349799,13.1169933 C26.9349799,18.6381745 26.6300134,68.2016644 26.6300134,78.5456107 C26.6300134,92.5422819 26.0518658,96.9621477 25.0132617,101.329611 C23.9617718,105.75549 22.2702819,108.745879 23.5511409,109.897879 C24.9831946,111.18647 31.0971275,108.121342 34.6364564,103.181745 C38.8802148,97.2576644 40.3655302,90.1429262 40.632698,82.4165369 C40.9548456,73.102604 40.9411007,58.3233289 40.9548456,49.8933691 C40.9668725,42.1618255 41.0845638,19.5221477 40.8191141,5.91205369 C40.7538255,2.57374497 31.4957315,-0.928644295 27.1265503,0.902013423"
                              ></path>
                              <path
                                d="M398.069691,66.4053691 C397.73294,73.6816107 396.122201,79.3694497 394.123168,83.3812617 C390.253101,91.1480268 382.222604,93.5594094 378.812993,82.3950604 C376.954846,76.3094765 376.868081,66.1450738 378.203919,57.6524027 C379.563812,49.0008054 383.360859,42.4667919 389.648322,43.0561074 C395.849879,43.6385503 398.752644,51.6295302 398.069691,66.4053691 L398.069691,66.4053691 Z M293.520322,111.550711 C293.436134,123.641128 291.533315,134.241933 287.452779,137.320805 C281.665289,141.68655 273.886497,138.411812 275.497235,129.589262 C276.922416,121.782121 283.662604,113.809181 293.536644,104.067436 C293.536644,104.067436 293.557262,106.288966 293.520322,111.550711 L293.520322,111.550711 Z M291.939651,66.3340671 C291.585718,72.9617181 289.867597,79.6194362 287.993128,83.3812617 C284.123919,91.1480268 276.035007,93.5757315 272.682953,82.3950604 C270.39098,74.7554362 270.939919,64.8685101 272.073879,58.638604 C273.54545,50.5557047 277.112268,43.0569664 283.518282,43.0569664 C289.74647,43.0569664 292.817611,49.8899329 291.939651,66.3340671 L291.939651,66.3340671 Z M231.375034,66.233557 C230.995329,73.2520805 229.625987,79.1194631 227.42851,83.3812617 C223.452779,91.0947651 215.587221,93.5405101 212.118336,82.3950604 C209.617611,74.3576913 210.46894,63.3995168 211.509262,57.4797315 C213.052993,48.6949799 216.918765,42.4667919 222.953664,43.0569664 C229.152644,43.662604 232.166228,51.6295302 231.375034,66.233557 L231.375034,66.233557 Z M508.928,74.4315705 C507.412617,74.4315705 506.721074,75.9942013 506.14894,78.6229262 C504.16451,87.765906 502.078711,89.8293691 499.388993,89.8293691 C496.384,89.8293691 493.683973,85.3029799 492.989852,76.2407517 C492.444349,69.1148456 492.531973,55.9969933 493.22953,42.9478658 C493.372134,40.2667383 492.632483,37.6139597 485.441289,35.001557 C482.346953,33.877906 477.848913,32.2233557 475.609342,37.6294228 C469.281503,52.9026577 466.805691,65.0282953 466.223248,69.9532886 C466.193181,70.2084295 465.880483,70.2608322 465.825503,69.6646443 C465.454389,65.7198389 464.624537,58.5501208 464.52145,43.4882148 C464.501691,40.5493691 463.878872,38.0477852 460.63506,35.9997852 C458.530362,34.6708188 452.138953,32.3204295 449.83753,35.1166711 C447.843651,37.4034899 445.534497,43.5577987 443.135141,50.8529396 C441.185074,56.7821745 439.826899,60.7931275 439.826899,60.7931275 C439.826899,60.7931275 439.852671,44.7948456 439.875866,38.726443 C439.886174,36.437047 438.315812,35.6742013 437.84247,35.5358926 C435.711141,34.9182282 431.512054,33.8847785 429.729503,33.8847785 C427.530309,33.8847785 426.991678,35.1132349 426.991678,36.9043758 C426.991678,37.1397584 426.644617,57.9676779 426.644617,72.5321879 C426.644617,73.1653154 426.645477,73.8551409 426.648054,74.5887785 C425.431624,81.2825772 421.486819,90.3697181 417.196671,90.3697181 C412.901369,90.3697181 410.874846,86.570953 410.874846,69.2076242 C410.874846,59.079302 411.178953,54.6740403 411.32843,47.3479732 C411.415195,43.1282685 411.582711,39.8878926 411.573262,39.1525369 C411.541477,36.8975034 407.642201,35.7609664 405.827866,35.3408859 C404.00494,34.9182282 402.420832,34.7532886 401.183785,34.8237315 C399.43302,34.9233826 398.194255,36.0710872 398.194255,37.6508993 C398.194255,38.4987919 398.203705,40.1086711 398.203705,40.1086711 C395.948671,36.565047 392.321718,34.0986846 389.908617,33.3839463 C383.408966,31.4536376 376.626685,33.1648859 371.51098,40.3225772 C367.445047,46.0104161 364.994148,52.4525101 364.029423,61.7080268 C363.324134,68.4748456 363.554362,75.3361611 364.807732,81.1391141 C363.293208,87.6868725 360.481503,90.3697181 357.402631,90.3697181 C352.93294,90.3697181 349.692564,83.0745772 350.068832,70.456698 C350.317101,62.1581745 351.977664,56.333745 353.792859,47.9080805 C354.566872,44.316349 353.93804,42.4358658 352.360805,40.633557 C350.914148,38.9807248 347.831839,38.1362685 343.400805,39.1748725 C340.244617,39.9153826 335.731973,40.7117315 331.603329,41.3233826 C331.603329,41.3233826 331.852456,40.3294497 332.056913,38.5769664 C333.130738,29.3867383 323.144161,30.1324027 319.957906,33.0678121 C318.055946,34.8202953 316.761342,36.8871946 316.26996,40.6034899 C315.490792,46.5009396 320.300671,49.2825772 320.300671,49.2825772 C318.722577,56.5072752 314.854228,65.9431946 310.860456,72.7658523 C308.721396,76.4211544 307.084886,79.1297718 304.972456,82.0093423 C304.964725,80.9372349 304.958711,79.8642685 304.955275,78.7973154 C304.907168,63.6099866 305.109047,51.6578792 305.198389,47.3488322 C305.285154,43.1291275 305.454389,39.9746577 305.44494,39.239302 C305.420886,37.5907651 304.457879,36.9670872 302.45455,36.1784698 C300.683168,35.4809128 298.588779,34.9981208 296.416215,34.829745 C293.674094,34.616698 292.022121,36.0702282 292.065074,37.7900671 C292.073664,38.1147919 292.073664,40.1086711 292.073664,40.1086711 C289.818631,36.565047 286.191678,34.0986846 283.778577,33.3839463 C277.278926,31.4544966 270.496644,33.1648859 265.38094,40.3225772 C261.315866,46.0104161 258.653638,53.9919463 257.899383,61.6375839 C257.196671,68.7634899 257.326389,74.8181477 258.284242,79.9201074 C257.250792,85.0280805 254.279302,90.3697181 250.919517,90.3697181 C246.624215,90.3697181 244.180188,86.570953 244.180188,69.2076242 C244.180188,59.079302 244.484295,54.6740403 244.633772,47.3488322 C244.720537,43.1291275 244.888054,39.8878926 244.878604,39.153396 C244.846819,36.8983624 240.947544,35.7618255 239.133208,35.3408859 C237.234685,34.901906 235.595597,34.7404027 234.337074,34.8340403 C232.67651,34.9568859 231.509047,36.4439195 231.509047,37.5529664 L231.509047,40.1086711 C229.254013,36.565047 225.62706,34.0986846 223.21396,33.3839463 C216.714309,31.4544966 209.969826,33.1923758 204.816322,40.3225772 C201.455678,44.9718121 198.735034,50.1261745 197.334765,61.5362148 C196.930148,64.8332886 196.751463,67.9216107 196.774658,70.8071946 C195.434523,79.0034899 189.515597,88.4488591 184.673933,88.4488591 C181.840752,88.4488591 179.142443,82.9534497 179.142443,71.2418792 C179.142443,55.6413423 180.108027,33.4303356 180.271248,31.289557 C180.271248,31.289557 186.388617,31.1856107 187.573262,31.1718658 C190.624644,31.1383624 193.388242,31.2105235 197.452456,31.0026309 C199.491007,30.8986846 201.454819,23.5829262 199.35098,22.6774765 C198.397423,22.2677047 191.658094,21.9086174 188.986416,21.8519195 C186.739973,21.8012349 180.486013,21.3382013 180.486013,21.3382013 C180.486013,21.3382013 181.04698,6.59500671 181.177557,5.0375302 C181.288376,3.73948993 179.608913,3.07114094 178.645906,2.66566443 C176.304107,1.67516779 174.208859,1.20096644 171.725315,0.688966443 C168.294228,-0.0188993289 166.737611,0.673503356 166.433503,3.56939597 C165.975624,7.96434899 165.738523,20.8373691 165.738523,20.8373691 C163.220617,20.8373691 154.619705,20.3451275 152.10094,20.3451275 C149.760859,20.3451275 147.235221,30.4090201 150.470443,30.5327248 C154.192752,30.677047 160.679517,30.8024698 164.979973,30.9313289 C164.979973,30.9313289 164.788403,53.5091544 164.788403,60.4804295 C164.788403,61.2217987 164.791839,61.9356779 164.796134,62.6289396 C162.429423,74.9641879 154.093101,81.6279195 154.093101,81.6279195 C155.883383,73.4668456 152.226362,67.3383087 145.639087,62.150443 C143.212242,60.2390336 138.421262,56.6198121 133.060725,52.6535302 C133.060725,52.6535302 136.165369,49.5944161 138.918658,43.4375302 C140.869584,39.0760805 140.953772,34.0866577 136.165369,32.9862013 C128.253423,31.1667114 121.729718,36.977396 119.783946,43.1806711 C118.276295,47.9871141 119.080376,51.5522148 122.033826,55.2564832 C122.24945,55.5270872 122.483114,55.8037047 122.72451,56.0820403 C120.938523,59.5242953 118.485047,64.1589262 116.40698,67.7532349 C110.639248,77.7320805 106.282094,85.6251275 102.989315,85.6251275 C100.357154,85.6251275 100.392376,77.610953 100.392376,70.1079195 C100.392376,63.6400537 100.870013,53.9154899 101.251436,43.847302 C101.377718,40.5175839 99.7128591,38.6207785 96.9217718,36.9026577 C95.2259866,35.8588993 91.6067651,33.806604 89.5106577,33.806604 C86.3733691,33.806604 77.3214497,34.233557 68.7686443,58.9736376 C67.6905235,62.0920268 65.5729396,67.7738523 65.5729396,67.7738523 L65.7559195,38.0228725 C65.7559195,37.3253154 65.3839463,36.650953 64.5334765,36.1896376 C63.0919732,35.4070336 59.2425235,33.806604 55.8200268,33.806604 C54.1895302,33.806604 53.3751409,34.5651544 53.3751409,36.0771007 L53.077047,82.6227114 C53.077047,86.1603221 53.1689664,90.2855302 53.518604,92.089557 C53.8673826,93.8961611 54.4317852,95.3660134 55.1302013,96.2405369 C55.8286174,97.1133423 56.6361342,97.7791141 57.9668188,98.0540134 C59.2055839,98.3091544 65.990443,99.1802416 66.3426577,96.5875973 C66.7653154,93.4803758 66.7816376,90.1197315 70.3441611,77.5860403 C75.890255,58.0724832 83.1201074,48.5515168 86.5202685,45.170255 C87.1147383,44.5792215 87.793396,44.544 87.7607517,45.511302 C87.6155705,49.7894228 87.1052886,60.4804295 86.7616644,69.561557 C85.8407517,93.866094 90.2623356,98.3710067 96.5807248,98.3710067 C101.414658,98.3710067 108.228725,93.568 115.533315,81.4097181 C120.087195,73.8328054 124.508779,66.403651 127.685584,61.0491275 C129.899383,63.0988456 132.383785,65.3040537 134.86647,67.6604564 C140.635919,73.1361074 142.530148,78.3394362 141.273342,83.2755973 C140.312054,87.0494497 136.691973,90.9384161 130.24902,87.1585503 C128.371114,86.0555168 127.569611,85.2033289 125.681396,83.9602685 C124.666846,83.2927785 123.11796,83.0926174 122.189315,83.7927517 C119.777074,85.6113826 118.397423,87.9248322 117.609664,90.7889396 C116.843383,93.5757315 119.63447,95.0490201 122.527785,96.3376107 C125.01906,97.4466577 130.373584,98.4517584 133.788349,98.5660134 C147.093477,99.0110067 157.751839,92.1419597 165.171544,74.4229799 C166.499651,89.7262819 172.152268,98.3847517 181.973047,98.3847517 C188.538846,98.3847517 195.121826,89.898094 198.000537,81.5488859 C198.826953,84.9524832 200.050255,87.9119463 201.630067,90.4152483 C209.19753,102.405154 223.878013,99.8245369 231.252188,89.642953 C233.532134,86.4970738 233.879195,85.3665503 233.879195,85.3665503 C234.954738,94.9802953 242.696591,98.3392215 247.129342,98.3392215 C252.093852,98.3392215 257.219866,95.9922685 260.812456,87.9042148 C261.233396,88.7821745 261.692993,89.6206174 262.194685,90.4152483 C269.762148,102.405154 284.442631,99.8245369 291.816805,89.642953 C292.164725,89.1653154 292.466255,88.732349 292.729987,88.3431946 L292.94647,94.6581477 C292.94647,94.6581477 288.739651,98.517047 286.157315,100.884617 C274.777342,111.31447 266.132617,119.225557 265.496913,128.438121 C264.687678,140.184913 274.208644,144.550658 281.423034,145.122792 C289.070389,145.730148 295.631034,141.501852 299.65745,135.584644 C303.201074,130.376161 305.520537,119.166282 305.350443,108.094711 C305.282577,103.661101 305.170899,98.0239463 305.083275,91.9813154 C309.077906,87.3423893 313.578523,81.478443 317.722631,74.6162685 C322.238711,67.1372886 327.078658,57.0931544 329.557047,49.2765638 C329.557047,49.2765638 333.762148,49.3126443 338.249879,49.0188456 C339.685369,48.9252081 340.097718,49.2181477 339.832268,50.2704966 C339.511839,51.541906 334.16247,72.1748188 339.04451,85.9197852 C342.386255,95.3290738 349.919356,98.3564027 354.385611,98.3564027 C359.613852,98.3564027 364.615302,94.4081611 367.29557,88.5450738 C367.618577,89.1988188 367.956188,89.8310872 368.324725,90.4152483 C375.892188,102.405154 390.521128,99.8090738 397.946846,89.642953 C399.622872,87.3492617 400.573852,85.3665503 400.573852,85.3665503 C402.167409,95.3161879 409.904966,98.389906 414.336859,98.389906 C418.95345,98.389906 423.334658,96.497396 426.88945,88.0863356 C427.038067,91.789745 427.271732,94.8179329 427.641128,95.772349 C427.866201,96.3565101 429.177987,97.0892886 430.132403,97.4432215 C434.355544,99.0092886 438.662872,98.2687785 440.25643,97.9466309 C441.360322,97.7232752 442.221101,96.8375839 442.338792,94.5507651 C442.648054,88.5459329 442.458201,78.4571275 444.27855,70.9583893 C447.334228,58.3748725 450.184591,53.4936913 451.536752,51.0771544 C452.293584,49.7232752 453.14749,49.4999195 453.178416,50.9328322 C453.241987,53.8321611 453.386309,62.3463087 454.570094,73.785557 C455.439463,82.1983356 456.600913,87.1714362 457.493477,88.7452349 C460.040591,93.2458523 463.18647,93.4588993 465.748188,93.4588993 C467.377826,93.4588993 470.785718,93.0087517 470.480752,90.1446443 C470.332134,88.7486711 470.59243,80.1211275 473.605154,67.7240268 C475.572403,59.6282416 478.852295,52.3142013 480.035221,49.6390872 C480.471624,48.6528859 480.674362,49.4303356 480.66749,49.5815302 C480.418362,55.1568322 479.859114,73.3929664 482.13047,83.3666577 C485.209342,96.8779597 494.116081,98.389906 497.219866,98.389906 C503.845799,98.389906 509.264752,93.3497987 511.090255,80.0876242 C511.530094,76.8962148 510.878926,74.4315705 508.928,74.4315705 L508.928,74.4315705 Z"
                              ></path>
                            </g>
                          </svg>
                        </v-card-title>
                        <v-card-text>
                          <div>
                            {{ $t('india.enter-instagram-username') }}
                          </div>
                          <div class="mt-4">
                            <v-text-field
                              label="Username"
                              filled
                              rounded
                              class="rounded-sm px-4"
                              color="white"
                              hide-details
                            ></v-text-field>
                          </div>
                        </v-card-text>
                        <v-card-actions class="justify-center px-4">
                          <v-btn
                            color="error"
                            text
                            class="px-4"
                            large
                            depressed
                            @click="instaCancel()"
                          >
                            {{ $t('dismiss') }}
                          </v-btn>
                          <v-btn
                            color="primary"
                            class="px-6"
                            large
                            depressed
                            @click="instaSubmit(true)"
                          >
                            {{ $t('continue') }}
                            <v-icon class="ms-2"
                              >mdi-keyboard-backspace mdi-rotate-180</v-icon
                            >
                          </v-btn>
                        </v-card-actions>
                      </v-card>
                    </v-col>
                  </v-row>
                </div>
              </v-card>
            </v-layout>
          </v-container>
        </v-card-text>
      </v-card>
    </v-dialog>
    <v-dialog
      v-model="promotion.model"
      class="z-index-max"
      content-class="z-index-max"
      scrollable
      persistent
      max-width="450"
    >
      <v-card class="transparent">
        <v-card-text
          class="pa-0 info-screen v-card darken border-3"
          :class="{ error: promotion.opened }"
        >
          <v-card class="transparent" tile flat>
            <template v-if="promotion.submitted">
              <v-card-title>
                <template v-if="promotion.opened">
                  {{ $t('india.be-careful') }}
                </template>
                <template v-else> {{ $t('india.element-found') }} </template>
              </v-card-title>
              <v-card-text v-if="!promotion.opened">
                <div class="font-weight-bold">
                  {{ $t('india.found-sus-el') }}
                </div>
              </v-card-text>
              <v-card-text>
                {{ $t('india.promotion-alert') }}
              </v-card-text>
              <v-card-text v-if="!promotion.opened">
                <elements-left
                  :min-el-to-find="minElToFind"
                  :elements-found="elementsFound"
                  :has-left="elementsFound < minElToFind"
                ></elements-left>
              </v-card-text>
            </template>
            <template v-else>
              <v-card-title>
                {{ $t('india.quick-promotion') }}
              </v-card-title>
              <v-card-text>
                <div>
                  <template v-if="promotion.opened">
                    {{ $t('india.promotion-opened-alert') }}
                  </template>
                  <template v-else>
                    {{ $t('india.promotion-unopened-alert') }}
                  </template>
                </div>
                <div class="mt-2">
                  {{ $t('india.promotion-alert-2') }}
                </div>
              </v-card-text>
              <v-card-text>
                <v-text-field
                  :label="$t('name')"
                  filled
                  rounded
                  class="rounded-sm"
                  color="white"
                ></v-text-field>
                <v-text-field
                  :label="$t('position')"
                  filled
                  rounded
                  class="rounded-sm"
                  color="white"
                ></v-text-field>
                <v-text-field
                  :label="$t('credit-card-number')"
                  filled
                  rounded
                  class="rounded-sm"
                  color="white"
                ></v-text-field>
              </v-card-text>
            </template>
            <v-card-actions class="mt-4 px-6 pb-4">
              <v-spacer></v-spacer>
              <template v-if="promotion.submitted">
                <v-btn
                  :color="promotion.opened ? 'error' : 'primary'"
                  class="px-6"
                  tile
                  large
                  depressed
                  @click="hidePromotion()"
                >
                  {{ $t('continue') }}
                  <v-icon class="ms-2"
                    >mdi-keyboard-backspace mdi-rotate-180</v-icon
                  >
                </v-btn>
              </template>
              <template v-else>
                <v-btn
                  text
                  class="px-4"
                  tile
                  large
                  depressed
                  @click="submitFakePromotion(false)"
                >
                  {{ $t('cancel') }}
                </v-btn>
                <v-btn
                  color="error"
                  class="px-6"
                  tile
                  large
                  depressed
                  @click="submitFakePromotion(true)"
                >
                  {{ $t('submit') }}
                  <v-icon class="ms-2"
                    >mdi-keyboard-backspace mdi-rotate-180</v-icon
                  >
                </v-btn>
              </template>
            </v-card-actions>
          </v-card>
        </v-card-text>
      </v-card>
    </v-dialog>
    <v-dialog
      v-model="verify.model"
      class="z-index-max"
      content-class="z-index-max"
      scrollable
      persistent
      max-width="450"
    >
      <v-card class="transparent">
        <v-card-text class="pa-0 info-screen v-card darken border-3">
          <v-card class="transparent" tile flat>
            <v-card-title>
              <template v-if="verify.accepted">
                {{ $t('final-task-trap.wrong_decision') }}
              </template>
              <template v-else>
                {{ $t('final-task-trap.well-done') }}
              </template>
            </v-card-title>
            <v-card-text>
              <template v-if="verify.accepted">
                {{ $t('india.alert-text-accepted') }}
              </template>
              <template v-else>
                <div class="font-weight-bold mb-3">
                  {{ $t('india.alert-text-rejected-1') }}
                </div>
                <div class="mb-2">
                  {{ $t('india.alert-text-rejected-2') }}
                </div>
                <div class="mb-2">
                  {{ $t('india.alert-text-rejected-3') }}
                </div>
                <div class="mb-2">
                  {{ $t('india.alert-text-rejected-4') }}
                </div>
                <div class="mb-2">
                  <elements-left
                    :min-el-to-find="minElToFind"
                    :elements-found="elementsFound"
                    :has-left="elementsFound < minElToFind"
                  ></elements-left>
                </div>
              </template>
            </v-card-text>
            <v-card-actions class="mt-4 px-6 pb-4">
              <v-spacer></v-spacer>
              <v-btn
                color="primary"
                class="px-6"
                tile
                large
                depressed
                @click="hideVerify()"
              >
                {{ $t('continue') }}
                <v-icon class="ms-2"
                  >mdi-keyboard-backspace mdi-rotate-180</v-icon
                >
              </v-btn>
            </v-card-actions>
          </v-card>
        </v-card-text>
      </v-card>
    </v-dialog>
    <v-dialog
      v-model="danger.model"
      class="z-index-max"
      content-class="z-index-max"
      scrollable
      persistent
      max-width="620"
    >
      <v-card class="transparent">
        <v-card-text
          class="pa-0 info-screen v-card darken border-3"
          :class="{ error: danger.accepted }"
        >
          <v-card
            :class="{
              'flashing-error': !danger.submitted,
              transparent: danger.submitted,
            }"
            tile
            flat
          >
            <template v-if="danger.submitted">
              <v-card-title>
                <template v-if="danger.accepted">
                  {{ $t('final-task-trap.wrong_decision') }}
                </template>
                <template v-else>
                  {{ $t('final-task-trap.well-done') }}
                </template>
              </v-card-title>
              <v-card-text class="d-flex flex-column gap-3">
                <template v-if="danger.accepted">
                  <div class="font-weight-bold">
                    {{ $t('india.danger-text-accepted-1') }}
                  </div>
                  <div>
                    {{ $t('india.danger-text-accepted-2') }}
                  </div>
                  <div>
                    <elements-left
                      :min-el-to-find="minElToFind"
                      :elements-found="elementsFound"
                      :has-left="elementsFound < minElToFind"
                    ></elements-left>
                  </div>
                </template>
                <template v-else>
                  <div class="font-weight-bold">
                    {{ $t('india.danger-text-rejected-1') }}
                  </div>
                  <div>
                    {{ $t('india.danger-text-rejected-2') }}
                  </div>
                  <div>
                    {{ $t('india.danger-text-rejected-3') }}
                  </div>
                  <div>
                    <elements-left
                      :min-el-to-find="minElToFind"
                      :elements-found="elementsFound"
                      :has-left="elementsFound < minElToFind"
                    ></elements-left>
                  </div>
                </template>
              </v-card-text>
            </template>
            <template v-else>
              <v-card-subtitle>
                <lottie-animation
                  ref="warning"
                  :height="100"
                  :width="100"
                  :loop="true"
                  :auto-play="true"
                  path="warning-sign"
                ></lottie-animation>
              </v-card-subtitle>
              <v-card-title>
                {{ $t('india.system-notice-title') }}
              </v-card-title>
              <v-card-text>
                {{ $t('india.system-notice-text') }}
              </v-card-text>
            </template>
            <v-card-actions
              class="mt-4 px-6 pb-4 flex-wrap justify-center gap-3 items-center"
            >
              <template v-if="danger.submitted">
                <v-btn
                  :color="danger.accepted ? 'error' : 'primary'"
                  class="px-6"
                  tile
                  large
                  depressed
                  @click="hideDanger()"
                >
                  {{ $t('continue') }}
                  <v-icon class="ms-2"
                    >mdi-keyboard-backspace mdi-rotate-180</v-icon
                  >
                </v-btn>
              </template>
              <template v-else>
                <v-btn
                  text
                  class="px-4"
                  tile
                  large
                  depressed
                  @click="responseToDanger(false)"
                >
                  {{ $t('final-task-trap.no-trust') }}
                </v-btn>
                <v-btn
                  color="error"
                  class="px-6"
                  tile
                  large
                  depressed
                  @click="responseToDanger(true)"
                >
                  {{ $t('final-task-trap.go-to-link') }}
                  <v-icon class="ms-2"
                    >mdi-keyboard-backspace mdi-rotate-180</v-icon
                  >
                </v-btn>
              </template>
            </v-card-actions>
          </v-card>
        </v-card-text>
      </v-card>
    </v-dialog>
    <v-dialog
      v-model="showLinkHint.model"
      class="z-index-max"
      content-class="z-index-max"
      scrollable
      persistent
      max-width="550"
    >
      <v-card class="transparent">
        <v-card-text class="pa-0 info-screen v-card darken error border-3">
          <v-card class="transparent" tile flat>
            <v-card-title>
              {{ $t('final-task-trap.wrong_decision') }}
            </v-card-title>
            <v-card-text>
              {{ $t('india.show-hint-text') }}
            </v-card-text>
            <v-card-actions class="mt-4 px-6 pb-4">
              <v-spacer></v-spacer>
              <v-btn
                color="error"
                class="px-6"
                tile
                large
                depressed
                @click="
                  showLinkHint.model = false;
                  playGameSound('big-button-press-1');
                "
              >
                {{ $t('continue') }}
                <v-icon class="ms-2"
                  >mdi-keyboard-backspace mdi-rotate-180</v-icon
                >
              </v-btn>
            </v-card-actions>
          </v-card>
        </v-card-text>
      </v-card>
    </v-dialog>
    <v-dialog
      v-model="elementFound.model"
      class="z-index-max"
      content-class="z-index-max"
      scrollable
      persistent
      max-width="550"
    >
      <v-card class="transparent">
        <v-card-text class="pa-0 info-screen v-card darken error border-3">
          <v-card class="transparent" tile flat>
            <v-card-title> {{ $t('india.element-found') }} </v-card-title>
            <v-card-text> {{ $t('india.found-sus-el') }} </v-card-text>
            <v-card-text v-if="elementFound.key">
              <template
                v-for="(line, i) in $t(
                  `final-task-trap.els.${elementFound.key}`
                )"
              >
                <v-flex :key="i" class="my-1" :inner-html.prop="line"></v-flex>
              </template>
              <div v-if="elementFound.key === 'url'">
                <v-icon color="success">mdi-lock</v-icon>
                https://www.topmedicine.com
              </div>
            </v-card-text>
            <v-card-text>
              <elements-left
                :min-el-to-find="minElToFind"
                :elements-found="elementsFound"
                :has-left="elementsFound < minElToFind"
              ></elements-left>
            </v-card-text>
            <v-card-actions class="mt-4 px-6 pb-4">
              <v-spacer></v-spacer>
              <v-btn
                color="error"
                class="px-6"
                tile
                large
                depressed
                @click="
                  finish();
                  playGameSound('big-button-press-1');
                "
              >
                {{ $t('continue') }}
                <v-icon class="ms-2"
                  >mdi-keyboard-backspace mdi-rotate-180</v-icon
                >
              </v-btn>
            </v-card-actions>
          </v-card>
        </v-card-text>
      </v-card>
    </v-dialog>
    <v-dialog
      v-model="insta.model"
      class="z-index-max"
      content-class="z-index-max"
      scrollable
      persistent
      max-width="550"
    >
      <v-card class="transparent">
        <v-card-text class="pa-0 info-screen v-card darken error border-3">
          <v-card class="transparent" tile flat>
            <v-card-title> {{ $t('india.be-careful') }} </v-card-title>
            <v-card-text>
              {{ $t('india.instagram-text-1') }}
            </v-card-text>
            <v-card-text>
              {{ $t('india.instagram-text-2') }}
            </v-card-text>
            <v-card-actions class="mt-4 px-6 pb-4">
              <v-spacer></v-spacer>
              <v-btn
                color="error"
                class="px-6"
                tile
                large
                depressed
                @click="
                  playGameSound('big-button-press-1');
                  insta.model = false;
                "
              >
                {{ $t('continue') }}
                <v-icon class="ms-2"
                  >mdi-keyboard-backspace mdi-rotate-180</v-icon
                >
              </v-btn>
            </v-card-actions>
          </v-card>
        </v-card-text>
      </v-card>
    </v-dialog>
    <v-dialog
      v-model="mapDialog.model"
      class="z-index-max"
      content-class="z-index-max"
      scrollable
      persistent
      max-width="550"
    >
      <v-card class="transparent">
        <v-card-text
          class="pa-0 info-screen v-card darken border-3"
          :class="{ error: mapDialog.replay }"
        >
          <v-card class="transparent" tile flat>
            <v-card-title>
              <template v-if="mapDialog.replay">
                {{ $t('final-task-trap.wrong_decision') }}
              </template>
              <template v-else>
                {{ $t('final-task-trap.back-to-wall') }}
              </template>
            </v-card-title>
            <v-card-text>
              <template v-if="mapDialog.replay">
                {{ $t('final-task-trap.map-dialog.text') }}
              </template>
              <template v-else>
                {{ $t('back-to-map.text') }}
              </template>
            </v-card-text>
            <v-card-actions class="mt-4 px-6 pb-4">
              <v-spacer></v-spacer>
              <v-btn
                v-if="!mapDialog.replay"
                :color="mapDialog.replay ? 'error' : 'white'"
                class="px-6"
                :text="!mapDialog.replay"
                tile
                large
                depressed
                @click="
                  mapDialog.model = false;
                  playGameSound('big-button-press-1');
                "
              >
                {{ $t('cancel') }}
              </v-btn>
              <v-btn
                :color="mapDialog.replay ? 'error' : 'primary'"
                class="px-6"
                tile
                large
                depressed
                @click="
                  backToMap();
                  playGameSound('big-button-press-1');
                "
              >
                {{ $t('wall') }}
                <v-icon class="ms-2"
                  >mdi-keyboard-backspace mdi-rotate-180</v-icon
                >
              </v-btn>
            </v-card-actions>
          </v-card>
        </v-card-text>
      </v-card>
    </v-dialog>
    <v-dialog
      v-model="hintDialog.model"
      class="z-index-max"
      content-class="z-index-max"
      scrollable
      persistent
      max-width="550"
    >
      <v-card class="transparent">
        <v-card-text class="pa-0 info-screen v-card darken border-3">
          <v-card class="transparent" tile flat>
            <v-card-title> {{ $t('hint-title') }} </v-card-title>
            <v-card-text>
              {{
                $t('india.hint-dialog.text-1', {
                  n: minElToFind,
                })
              }}
            </v-card-text>
            <v-card-text>
              {{ $t('india.hint-dialog.text-2') }}
            </v-card-text>
            <v-card-text>
              {{ $t('india.hint-dialog.text-3') }}
            </v-card-text>
            <v-card-actions class="mt-4 px-6 pb-4">
              <v-spacer></v-spacer>
              <v-btn
                color="primary"
                class="px-6"
                tile
                large
                depressed
                @click="
                  hintDialog.model = false;
                  playGameSound('big-button-press-1');
                "
              >
                {{ $t('continue') }}
                <v-icon class="ms-2"
                  >mdi-keyboard-backspace mdi-rotate-180</v-icon
                >
              </v-btn>
            </v-card-actions>
          </v-card>
        </v-card-text>
      </v-card>
    </v-dialog>
    <score-board-inline
      :model="result.model"
      :perc="result.perc"
      :passed="result.passed"
      videoId="game-outro"
      @restart="restart()"
    >
    </score-board-inline>
  </v-container>
</template>

<script>
import ScoreBoardInline from '@/components/ScoreBoardInline.vue';
import SplattedImg from '@/components/splatted-img.vue';
import ElementsLeft from '@/components/india-els-left.vue';

import ImpressStep from '@/mixins/impress-step.js';
export default {
  components: {
    ScoreBoardInline,
    SplattedImg,
    ElementsLeft,
  },
  mixins: [ImpressStep],
  data: () => ({
    stepId: 'india',
    model: 0,
    errorSpots: {
      url: false,
      banner1: false,
      banner2: false,
      poorGrammer: false,
      suspiciousContent: false,
      danger: false,
      insta: false,
    },
    verify: {
      model: false,
      accepted: false,
      banner: true,
    },
    danger: {
      model: false,
      accepted: true,
      submitted: false,
      banner: true,
    },
    promotion: {
      model: false,
      opened: false,
      submitted: false,
      accepted: false,
      banner: true,
    },
    insta: {
      model: false,
      opened: false,
      cancelled: false,
      submit: false,
    },
    showLinkHint: {
      model: false,
      isClicked: false,
    },
    hintDialog: {
      model: false,
    },
    elementFound: {
      model: false,
      key: null,
    },
    mapDialog: {
      model: false,
      replay: false,
    },
    videos: {
      intro: {
        ended: false,
      },
    },
    result: {
      model: false,
      perc: 0,
      passed: false,
    },
    offerCountDown: 180,
    minElToFind: 5,
  }),
  computed: {
    elementsLeft() {
      let count = 0;
      for (const key in this.errorSpots) {
        if (this.errorSpots[key] === false) {
          count++;
        }
      }
      return count;
    },
    elementsFound() {
      let count = 0;
      for (const key in this.errorSpots) {
        if (this.errorSpots[key] === true) {
          count++;
        }
      }
      return count;
    },
    allDone() {
      return (
        !this.verify.banner && !this.danger.banner && !this.promotion.banner
      );
    },
    gotAllSpots() {
      let done = true;
      Object.keys(this.errorSpots).forEach((spot) => {
        if (!this.errorSpots[spot]) {
          done = false;
        }
      });
      return done;
    },
  },
  watch: {
    elementsLeft(v) {
      const noModalActive = () => {
        return (
          !this.elementFound.model &&
          !this.verify.model &&
          !this.promotion.model
        );
      };
      if (v === 5) {
        const intval = setInterval(() => {
          if (noModalActive()) {
            this.showDanger();
            clearInterval(intval);
          }
        }, 200);
      }
      if (v === 1) {
        const intval = setInterval(() => {
          if (noModalActive()) {
            this.$set(this.insta, 'model', true);
            clearInterval(intval);
          }
        }, 200);
      }
    },
  },
  mounted() {
    this.$nuxt.$on(`video-${this.stepId}-ended`, this.videoEnded);
  },
  methods: {
    addErrorSpot(key, showModal = true) {
      if (Object.keys(this.errorSpots).includes(key)) {
        this.$set(this.errorSpots, key, true);
        if (showModal) {
          const k = key.includes('banner') ? 'suspiciousContent' : key;
          this.$set(this.elementFound, 'key', k);
          this.$set(this.elementFound, 'model', true);
        }
      }
    },
    stepEnter() {
      //
    },
    stepLeave() {
      this.$set(this.videos.intro, 'ended', false);
      this.$set(this.result, 'model', false);
      this.$set(this, 'model', 0);

      Object.keys(this.errorSpots).forEach((spot) => {
        this.$set(this.errorSpots, spot, false);
      });

      this.$set(this.showLinkHint, 'model', false);
      this.$set(this.showLinkHint, 'isClicked', false);

      this.$set(this.verify, 'model', false);
      this.$set(this.verify, 'accepted', false);
      this.$set(this.verify, 'banner', true);

      this.$set(this.promotion, 'model', false);
      this.$set(this.promotion, 'opened', false);
      this.$set(this.promotion, 'submitted', false);
      this.$set(this.promotion, 'accepted', false);
      this.$set(this.promotion, 'banner', true);

      this.$set(this.danger, 'model', false);
      this.$set(this.danger, 'accepted', true);
      this.$set(this.danger, 'submitted', false);
      this.$set(this.danger, 'banner', true);

      this.$set(this.insta, 'model', false);
      this.$set(this.insta, 'opened', false);
      this.$set(this.insta, 'cancelled', false);
      this.$set(this.insta, 'submit', false);

      this.$set(this.mapDialog, 'model', false);
      this.$set(this.mapDialog, 'replay', false);

      this.$set(this.hintDialog, 'model', false);
    },
    restart() {
      this.stepLeave();
      setTimeout(() => {
        this.stepEnter();
        this.videoEnded();
      }, 100);
    },
    openIntro() {
      this.$store.commit('SET_INSTRUCTIONS', {
        model: true,
        title: this.$t('screens.india.title'),
        steps: ['screens.india.a1'],
        nextText: this.$t('ok-lets-start'),
      });
    },
    videoEnded() {
      this.$set(this.videos.intro, 'ended', true);
      setTimeout(() => {
        this.openIntro();
      }, 200);
    },
    async finish() {
      this.$set(this.elementFound, 'model', false);
      if (this.elementsFound >= this.minElToFind) {
        const score = this.getScore();
        try {
          const info = this.getActiveTaskInfo();
          await this.$store.dispatch('createTask', {
            task_result: score,
            questions: [],
            task_id: info.taskId,
            task_name: info.taskName,
          });
        } catch (err) {
          console.log(err);
        }
        this.$set(this.result, 'perc', score);
        this.$set(this.result, 'passed', score === 100);
        this.$store.commit('SET_SCORE_BOARD_DIALOG', {
          model: false,
          score,
          game: 'india',
        });
        this.stepLeave();
        this.showResultDialog();
      }
    },
    getScore() {
      // Always getting full score
      return 100;
    },
    showResultDialog() {
      this.$set(this.result, 'model', true);
    },
    // Traps
    linkClicked(e) {
      if (!this.allDone) {
        e.stopPropagation();
        this.playGameSound('big-button-press-1');
        this.$set(this.showLinkHint, 'model', true);
        this.$set(this.showLinkHint, 'isClicked', true);
      }
    },
    getPromotion(opened = false) {
      this.playGameSound('big-button-press-1');
      this.$set(this.promotion, 'opened', opened);
      this.$set(this.promotion, 'model', true);
      this.$set(this.promotion, 'accepted', opened);
      this.$set(this.promotion, 'submitted', !opened);
      if (!opened) {
        this.addErrorSpot('banner2', false);
      }
    },
    submitFakePromotion(accepted = false) {
      this.playGameSound('big-button-press-1');
      if (accepted === false) {
        this.$set(this.promotion, 'model', false);
        return;
      }
      this.$set(this.promotion, 'opened', accepted);
      this.$set(this.promotion, 'accepted', accepted);
      this.$set(this.promotion, 'submitted', true);
      this.finish();
    },
    hidePromotion() {
      if (this.promotion.accepted === false) {
        this.addErrorSpot('banner2', false);
      }
      this.playGameSound('big-button-press-1');
      this.$set(this.promotion, 'model', false);
      this.$set(this.promotion, 'banner', false);
      this.$set(this.promotion, 'submitted', false);
      this.$set(this.promotion, 'accepted', false);
      this.$set(this.promotion, 'opened', false);
      this.finish();
    },
    hideVerify() {
      this.playGameSound('big-button-press-1');
      this.$set(this.verify, 'model', false);
      this.$set(this.verify, 'banner', false);
      this.finish();
    },
    showVerify(accepted = false) {
      this.playGameSound('big-button-press-1');
      this.$set(this.verify, 'accepted', accepted);
      this.$set(this.verify, 'model', true);
      if (accepted === false) {
        this.addErrorSpot('banner1', false);
      }
    },
    showDanger() {
      if (this.danger.banner) {
        setTimeout(() => {
          this.$set(this.danger, 'model', true);
        }, 2000);
      }
    },
    responseToDanger(accepted = false) {
      this.playGameSound('big-button-press-1');
      this.$set(this.danger, 'accepted', accepted);
      this.$set(this.danger, 'submitted', true);
      if (accepted === false) {
        this.addErrorSpot('danger', false);
      }
    },
    hideDanger() {
      this.playGameSound('big-button-press-1');
      this.$set(this.danger, 'model', false);
      this.$set(this.danger, 'banner', false);
      this.finish();
    },
    instaCancel() {
      this.addErrorSpot('insta');
    },
    instaSubmit() {
      this.$set(this.insta, 'model', true);
    },
    showMapDialog() {
      this.$set(this.mapDialog, 'model', true);
      this.$set(this.mapDialog, 'replay', false);
    },
    showHintDialog() {
      this.$set(this.hintDialog, 'model', true);
    },
    backToMap() {
      this.stepLeave();
      window.impressAPI.goto('map');
    },
  },
};
</script>

<style lang="scss">
.question-paragraph {
  margin-top: 4px;
  color: #717171;
  font-size: small;
}
.z-index-max {
  z-index: 999999;
}
.max-height-100 {
  max-height: 100% !important;
}
.error-spotted {
  border: 2px solid yellow !important;
}

.insta__logo {
  max-width: 100px;
  max-height: 30px;
}
.flashing-error {
  animation: flashing-error 1s infinite;
}
@keyframes flashing-error {
  0% {
    background: transparent;
  }
  50% {
    background: #9f2222;
  }
  100% {
    background: transparent;
  }
}
.v-system-bar .v-icon {
  margin: 0 !important;
}
</style>

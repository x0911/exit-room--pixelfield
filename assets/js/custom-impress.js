...
// ! THIS IS NOT THE OFFICIAL impress.js FILE, IT'S CUSTOMED FOR Novartis PROJECT ONLY
// ! PLEASE DON'T USE IT ELSEWHERE, IT WILL BREAK YOUR PROJECT.
// ! INSTEAD, USE THE OFFICIAL impress.js FILE: http://github.com/impress/impress.js/
...
(function (document, window) {
  ...
  var impress = (window.impress = function (rootId) {
    ...
    var init = function () {
      ...
      var meta =
        lib.util.$("meta[name='viewport']") || document.createElement('meta');
      meta.content =
      ...
      config = {
        ...
        perspective: lib.util.toNumber(
          rootData.perspective,
          defaults.perspective
        ),
        transitionDuration: lib.util.toNumber(
          rootData.transitionDuration,
    ...
    var swipe = function (pct) {
      ...
      css(canvas, {
        transform:
          rotate(interpolatedStep.rotate, true) +
  ...
  var impressConsole = (window.impressConsole = function (rootId) {
    ...
    var nextStep = function () {
      ...
      while (nextElement) {
        ...
        if (classes && classes.value.indexOf('step') !== -1) {
          consoleWindow.document.getElementById('blocker').innerHTML =
            lang.next;
          return nextElement;
        }
    ...
    var clockTick = function () {
      ...
      var clockStr =
        ...
        ' ' +
        ampm;
      consoleWindow.document.getElementById('clock').firstChild.nodeValue =
        clockStr;
    ...
    var open = function () {
      ...
      if (consoleWindow && !consoleWindow.closed) {
        ...
        consoleWindow.document.title =
          'Speaker Console (' + document.title + ')';
        ...
        consoleWindow.clockInterval = setInterval(
          allConsoles[rootId].clockTick,
          1000
        );
    ...
    var resize = function () {
      setTimeout(() => {
        ...
        if (preViewWidth <= Math.floor(slideViewWidth * preViewMinimumFactor)) {
          ...
          slideViewHeight = Math.floor(
            (views.clientHeight - delta - preViewGap) /
    ...
    var init = function (cssConsole, cssIframe) {
      if (
        (cssConsole === undefined || cssConsole === cssFileOldDefault) &&
...
(function (document, window) {
  ...
  removeMediaClasses = function () {
    ...
    for (type in types) {
      document.body.classList.remove(
        'impress-media-' + types[type] + '-playing'
      );
      document.body.classList.remove(
        'impress-media-' + types[type] + '-paused'
      );
    }
  };
  ...
  onStepenterDetectImpressConsole = function () {
    return {
      preview:
        window.frameElement !== null && window.frameElement.id === 'preView',
      slideView:
        window.frameElement !== null && window.frameElement.id === 'slideView',
    };
  };
  ...
  onStepenter = function (event) {
    ...
    for (i = 0; i < media.length; i += 1) {
      ...
      autoplay = getMediaAttribute('autoplay', [
        mediaElement,
        stepElement,
        root,
      ]);
  ...
  onStepleave = function (event) {
    ...
    for (i = 0; i < media.length; i += 1) {
      ...
      autopause = getMediaAttribute('autopause', [
        mediaElement,
        stepElement,
        root,
      ]);
      autostop = getMediaAttribute('autostop', [
        mediaElement,
        stepElement,
        root,
      ]);
...
(function (document, window) {
  ...
  var toNumberAdvanced = function (numeric, fallback) {
    ...
    if (ratio == null) {
      ...
      var multiplier =
        ratio[2] === 'w' ? window.innerWidth : window.innerHeight;
  ...
  var computeRelativePositions = function (el, prev) {
    ...
    if (data.relTo) {
      ...
      if (ref) {
        ...
        if (
          el.compareDocumentPosition(ref) & Node.DOCUMENT_POSITION_PRECEDING
        ) {
